...
func (m *AmpModule) registerProviderAliases(engine *gin.Engine, baseHandler *handlers.BaseAPIHandler, auth gin.HandlerFunc) {
	...
	provider.POST("/moderations", fallbackHandler.WrapHandler(openaiHandlers.Moderations))
	...
	v1Amp := provider.Group("/v1")
	{
		...
		v1Amp.POST("/moderations", fallbackHandler.WrapHandler(openaiHandlers.Moderations))
